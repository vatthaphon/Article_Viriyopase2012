...
% Date:  17 July, 2009                                              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function val=TimeBased_3MS_osc_PRC(p_phi1, p_phi2, p_tau1, p_tau3, p_epsi12, p_epsi21, p_epsi32, p_epsi23, p_tEnd, p_PRC_option)
global figOption;
%%%%%% Parameters %%%%%%
option=figOption;
b=3;            
...
for t=tstart:dt:tend
    ...
    if ( (a_time1<=time1) && (a_time3<=time3) && (a_time1>0) && (a_time3>0) && (t>=SpTi1(1,a_time1)+tau1-dt/2 && t< SpTi1(1,a_time1)+tau1+dt/2) && (t>=SpTi3(1,a_time3)+tau3-dt/2 && t< SpTi3(1,a_time3)+tau3+dt/2))
        phi(2)=phi(2)+dt+PRC(phi(2)+dt,epsi12+epsi32,b,p_PRC_option);
        a_time1=a_time1+1;
        a_time3=a_time3+1;
    elseif ((a_time1<=time1) && (a_time1>0) && t>=SpTi1(1,a_time1)+tau1-dt/2 && t< SpTi1(1,a_time1)+tau1+dt/2)
        phi(2)=phi(2)+dt+PRC(phi(2)+dt,epsi12,b,p_PRC_option);        
        a_time1=a_time1+1;
    elseif ((a_time3<=time3) && (a_time3>0) && t>=SpTi3(1,a_time3)+tau3-dt/2 && t< SpTi3(1,a_time3)+tau3+dt/2)
        phi(2)=phi(2)+dt+PRC(phi(2)+dt,epsi32,b,p_PRC_option);        
        a_time3=a_time3+1;
    else
        phi(2)=phi(2)+dt;
    end               
    ...
    if ((a_time21<=time2) && (a_time21>0) && t>=SpTi2(1,a_time21)+tau1-dt/2 && t< SpTi2(1,a_time21)+tau1+dt/2) % neuron 1 receives spike from neuron 2
        phi(1)=phi(1)+dt+PRC(phi(1)+dt,epsi21,b,p_PRC_option);
        a_time21=a_time21+1;
    else
        phi(1)=phi(1)+dt;
    ...
    if ((a_time23<=time2) && (a_time23>0) && t>=SpTi2(1,a_time23)+tau3-dt/2 && t< SpTi2(1,a_time23)+tau3+dt/2) % neuron 1 receives spike from neuron 2
        phi(3)=phi(3)+dt+PRC(phi(3)+dt,epsi23,b,p_PRC_option);        
        a_time23=a_time23+1;
    else
        phi(3)=phi(3)+dt;
    end  % if osc 1 receives pulse from osc 2
...
%     title('Dynamics of V_{Membrane}')
    ...
    plot(tstart:dt:tend,f(3,:),'r','LineWidth',2), hold on
    for i=2: time3
        plot([SpTi3(1,i) SpTi3(1,i)],[0 1],'r','LineWidth',2)
    end
%     plot(tstart:dt:tend,f(1,:),'LineWidth',0.5), hold on
%     for i=2: time1
%         plot([SpTi1(1,i) SpTi1(1,i)],[0 1],'LineWidth',0.5)
%     end
%     plot(tstart:dt:tend,f(2,:),'c','LineWidth',0.5), hold on
%     for i=2: time2
%         plot([SpTi2(1,i) SpTi2(1,i)],[0 1],'c','LineWidth',0.5)
%     end
%     plot(tstart:dt:tend,f(3,:),'r','LineWidth',0.5), hold on
%     for i=2: time3
%         plot([SpTi3(1,i) SpTi3(1,i)],[0 1],'r','LineWidth',0.5)
%     end
    grid on
%     maximize('all');
end
