...
global r_Period s_SecondHalf isDeBug

tend=1000;

phi1_min=0.5;phi1_max=phi1_min;phi1len=1;
phi2_min=0.948;phi2_max=phi2_min;phi2len=1;

tau1_min=0.2;tau1_max=tau1_min;tau1len=1;
tau3_min=0.2;tau3_max=tau3_min;tau3len=1;
            ...
            for k=1:1:tau1len
                ...
                if isSymE
                    for m=1:1:epsi12len
                        ...
                        epsi32=epsi12;
                        epsi23=epsi12;
retStr=EventBased_3MS_osc(init_phi1, init_phi2, tau1, tau3, epsi12, epsi21, epsi32, epsi23, tend, PRC_option);                        
...
% strcat1=['[1,3 n-driven + 2   driven]',' 1,3 vs 2:',num2str(tau1_min),'<lag<',num2str(1-tau1_min),' new period:', num2str(1-alpha(epsi12_min,3)-2*beta(epsi12_min,3)*tau1_min)];
% strcat2=['[1,3   driven + 2 n-driven]',' 1,3 vs 2:',num2str(tau1_min),' new period:', num2str(1-alpha(2*epsi12_min,35)-2*beta(2*epsi12_min,3)*tau1_min)];
% disp(strcat);
% disp(strcat1);
% disp(strcat2);
